...
import de.fraunhofer.iosb.svs.analysishub.data.service.SwrlRuleService;
...
public class AddSwrlRuleView extends Div {
    ...
    private final Binder<SwrlRule> binder = new Binder<>(SwrlRule.class);
    ...
    public AddSwrlRuleView(PhaseService phaseService,
        ...
        binder.forField(name).asRequired("Name needed").withValidator(new RegexpValidator("No whitespaces allowed", "^[a-zA-Z0-9\\-_]*")).bind(SwrlRule::getLocalName, SwrlRule::setUriByLocalName);
        binder.bind(description, SwrlRule::getDescription, SwrlRule::setDescription);
        binder.forField(phase).asRequired("Phase needed").bind(swrlRule -> phaseService.getStaticPhaseByPhase(swrlRule.getPhase()),
                (swrlRule, staticPhase) -> swrlRule.setPhase(phaseService.getPhaseByStaticPhase(staticPhase)));
        binder.bind(dependsOnImplementation,
                swrlRule -> swrlRule.getDependsOnImplementation().stream()
                        .map(implementation -> new ResourceUriProjection(implementation.getId(), implementation.getUri()))
                        .collect(Collectors.toSet()),
                (swrlRule, resourceUriProjections) -> swrlRule.setDependsOnImplementation(
        ...
        binder.bind(dependsOnOntologyDependency,
                swrlRule -> swrlRule.getDependsOnOntologyDependency().stream()
                        .map(ontologyDependency -> new ResourceUriProjection(ontologyDependency.getId(), ontologyDependency.getUri()))
                        .collect(Collectors.toSet()),
                (swrlRule, resourceUriProjections) -> swrlRule.setDependsOnOntologyDependency(
                        resourceUriProjections.stream()
                                .map(resourceUriProjection -> ontologyDependencyService.getOrThrow(resourceUriProjection.getId()))
                                .collect(Collectors.toList())
                ));
        binder.forField(rule).asRequired("Rule needed").bind(SwrlRule::getRule, SwrlRule::setRule);
        ...
        save.addClickListener(e -> {
            if (binder.validate().isOk()) {
                try {
                    swrlRuleService.update(binder.getBean());
